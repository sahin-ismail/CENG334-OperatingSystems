...
void assignDrone(Drone** droneList,int index,PackageInfo package, unsigned int call){
    pthread_mutex_lock(&droneList[index]->mutexCall);
...
int callDrone(unsigned int *distances ,unsigned int hubID, Drone** droneList, unsigned int droneCount, Hub** hubList){
    ...
    if(index==-1){
        ...
        printf("drone transfer ediliyor.............droneid: %u ....çağıran hubid : %u ..... gönderen-hubid : %u\n",droneList[index]->ID,droneList[index]->assignedHubId, droneList[index]->startingHubId );
        printf("index..... : %u\n",index );
        printf("alan hub in count u : %u........\n", hubList[droneList[index]->assignedHubId-1]->chargingSpaceCount);
...
void* senderRoutine(void *args){
    ...
    while(sender->numberOfPackages>0){
        ...
        printf("sender id:: %u  hub id:: %u  count :: %u\n",sender->ID, sender->assignedHubId, sender->numberOfPackages);
...
void* receiverRoutine(void *args){
    ...
    while(existActiveHub(receiver->hubList,receiver->hubCount)){
        ...
        printf("receiver alındıııı............................................................................id : %u\n",receiver->assignedHubId);
        for (int i = 0; i < receiver->hubCount; ++i)
        {
            /* code */
            printf("incoming %u\n", (receiver->hubList[i]->incomingSender - receiver->hubList[i]->incomingPackageCount));
            printf("outgoing %u\n", (receiver->hubList[i]->outgoingSender - receiver->hubList[i]->outgoingPackageCount));
        }
        //işlemler buraya
...
void* droneRoutine(void *args){
    ...
    while(existActiveHub(drone->hubList,drone->hubCount)){
        ...
        if(drone->call == 1){
            ...
            printf("drone paketleri götürdü..................gönderen-hubid : %u....alan-hubid : %u......droneid : %u\n",drone->package.sending_hub_id,drone->startingHubId,drone->ID);
            drone->call = 0;
            ...
            printf("drone transfer oldu..................gönderen-hubid : %u....alan-hubid : %u.....dronid : %u\n",drone->startingHubId,drone->assignedHubId,drone->ID);
...
void* hubRoutine(void *args){
    ...
    while(((hub->incomingSender - hub->incomingPackageCount)+hub->reservedCount)>0 || (hub->outgoingSender - hub->outgoingPackageCount)>0 || isActiveSenderFunc(hub->hubList,hub->hubCount,hub->droneCount,hub->droneList) == 1){ //ve aktif sender yoksa
        ...
        if(index!=-1){
            ...
            assignDrone(hub->droneList,index, hub->outgoing[hub->outgoingPackageCount],1);
            printf("drone paket götürüyor....giden-hubid: %u.....gidenhubactive : %u .......gonderen-hubid : %u .....droneid: %u \n", hub->droneList[index]->package.receiving_hub_id,hub->hubList[hub->droneList[index]->package.receiving_hub_id - 1]->isActiveHub,hub->ID, hub->droneList[index]->ID);
            printf("gidilen hub ın chargingSpace i : %u...incomingcount u.......%u \n",(hub->hubList[hub->droneList[index]->package.receiving_hub_id-1]->chargingSpaceNum - hub->hubList[hub->droneList[index]->package.receiving_hub_id-1]->chargingSpaceCount), (hub->hubList[hub->droneList[index]->package.receiving_hub_id-1]->incomingSender - hub->hubList[hub->droneList[index]->package.receiving_hub_id-1]->incomingPackageCount));
            hub->outgoingPackageCount = hub->outgoingPackageCount + 1;
...
int main() {
    ...
    for (unsigned int i = 0; i < hubCount; i++) {
        ...
        hub->incoming = malloc(500*sizeof(PackageInfo));
        hub->outgoing = malloc(500*sizeof(PackageInfo));
    ...
    /*for (int i = 0; i < 5; ++i)
    {
        printf("sender : %u\n",hubList[0]->outgoing[i].sender_id);
        printf("receiver : %u\n",hubList[0]->outgoing[i].receiver_id);
        printf("sender : %u\n",hubList[1]->outgoing[i].sender_id);
        printf("receiver : %u\n",hubList[1]->outgoing[i].receiver_id);
        printf("sender : %u\n",hubList[2]->outgoing[i].sender_id);
        printf("receiver : %u\n",hubList[2]->outgoing[i].receiver_id);
    }*/
    ...
    //printData(hubCount,droneCount,hubList,senderList,receiverList,droneList);
...
/*void printData(unsigned int hubCount, unsigned int droneCount, Hub** hubList, Sender** senderList, Receiver** receiverList, Drone** droneList){
    printf("%u\n", hubCount);
    for (int i = 0; i < hubCount; ++i)
    {
        printf("%u %u %u",hubList[i]->incomingPackageStorage,hubList[i]->outgoingPackageStorage,hubList[i]->chargingSpaceNum );
        for (int j = 0; j < hubCount; ++j)
        {
            printf(" %u",hubList[i]->distances[j]);
        }
        printf("\n");
    }
    ...
    for (int i = 0; i < hubCount; ++i)
    {
        printf("%u %u %u\n",senderList[i]->waitTime,senderList[i]->assignedHubId,senderList[i]->numberOfPackages );
    }

    for (int i = 0; i < hubCount; ++i)
    {
        printf("%u %u\n",receiverList[i]->waitTime,receiverList[i]->assignedHubId);
    }
    ...
    for (int i = 0; i < droneCount; ++i)
    {
        printf("%u %u %u\n",droneList[i]->travelSpeed,droneList[i]->startingHubId,droneList[i]->maxRange);
    }
